...
BEGIN
  ...
  LOOP
    ...
    IF v_row.outcome ILIKE '%active%' OR v_row.outcome ILIKE '%ongoing%' OR v_row.outcome ILIKE '%pending%' THEN
      ...
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'elc_case', 'reason', format('Active/ongoing case %s — %s', v_row.case_number, LEFT(v_row.parties, 80)), 'deduction', -25
      ));
    ELSIF v_row.outcome ILIKE '%against%' OR v_row.outcome ILIKE '%guilty%' OR v_row.outcome ILIKE '%favour of plaintiff%' THEN
      v_score := v_score - 20;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'elc_case', 'reason', format('Judgment against owner in case %s', v_row.case_number), 'deduction', -20
      ));
    ELSE
      v_score := v_score - 10;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'elc_case', 'reason', format('Historical case %s (resolved)', v_row.case_number), 'deduction', -10
      ));
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF v_row.alert_level = 'critical' THEN
      v_score := v_score - 30;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'gazette_notice', 'reason', format('Critical gazette notice: %s', LEFT(COALESCE(v_row.description, v_row.notice_type), 80)), 'deduction', -30
      ));
    ELSIF v_row.alert_level = 'high' THEN
      v_score := v_score - 20;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'gazette_notice', 'reason', format('High-alert gazette notice: %s', LEFT(COALESCE(v_row.description, v_row.notice_type), 80)), 'deduction', -20
      ));
    ELSIF v_row.alert_level = 'medium' THEN
      v_score := v_score - 10;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'gazette_notice', 'reason', format('Medium-alert gazette notice: %s', LEFT(COALESCE(v_row.description, v_row.notice_type), 80)), 'deduction', -10
      ));
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    IF v_row.status = 'verified' AND v_row.severity = 'high' THEN
      v_score := v_score - 20;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'community_flag', 'reason', format('Verified high-severity flag: %s in %s', v_row.category, v_row.county), 'deduction', -20
      ));
    ELSIF v_row.status = 'verified' AND v_row.severity = 'medium' THEN
      v_score := v_score - 10;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'community_flag', 'reason', format('Verified flag: %s in %s', v_row.category, v_row.county), 'deduction', -10
      ));
    ELSE
      v_score := v_score - 5;
      v_breakdown := v_breakdown || jsonb_build_array(jsonb_build_object(
        'source', 'community_flag', 'reason', format('Unverified flag: %s in %s', v_row.category, v_row.county), 'deduction', -5
      ));
    END IF;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'elc_cases_found', v_elc_count, 'gazette_notices_found', v_gazette_count,
    'community_flags_found', v_flags_count, 'last_calculated', NOW(),
