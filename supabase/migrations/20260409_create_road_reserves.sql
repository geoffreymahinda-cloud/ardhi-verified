...
CREATE INDEX idx_road_reserves_name_trgm
    ON road_reserves USING GIN (road_name gin_trgm_ops);

CREATE INDEX idx_road_reserves_route_trgm
    ON road_reserves USING GIN (route_description gin_trgm_ops);

CREATE INDEX idx_road_reserves_counties_gin
    ON road_reserves USING GIN (counties jsonb_ops);

CREATE INDEX idx_road_reserves_class
