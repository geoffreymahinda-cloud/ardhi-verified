      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            ...
            <ul className="space-y-2">
              {navigate.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/70 transition-colors hover:text-ardhi"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 sm:flex-row">
          <p className="text-xs text-white/40">
            &copy; {new Date().getFullYear()} Ardhi Verified. All rights reserved.
          </p>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
          >
            <svg
              className="h-4 w-4"
              fill="currentColor"
